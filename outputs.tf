...
output "vpc_arn" {
  ...
  value       = var.vpc_id != null ? concat(data.aws_vpc.existing.*.arn, [""])[0] : concat(aws_vpc.this.*.arn, [""])[0]
}

output "vpc_cidr_block" {
  description = "The CIDR block of the VPC"
  value       = var.vpc_id != null ? concat(data.aws_vpc.existing.*.cidr_block, [""])[0] : concat(aws_vpc.this.*.cidr_block, [""])[0]
}
...
output "default_network_acl_id" {
  ...
  value       = var.default_network_acl_id != null ? var.default_network_acl_id : concat(aws_vpc.this.*.default_network_acl_id, [""])[0]
}
...
output "vpc_instance_tenancy" {
  ...
  value       = var.vpc_id != null ? concat(data.aws_vpc.existing.*.instance_tenancy, [""])[0] : concat(aws_vpc.this.*.instance_tenancy, [""])[0]
}

output "vpc_enable_dns_support" {
  description = "Whether or not the VPC has DNS support"
  value       = var.vpc_id != null ? concat(data.aws_vpc.existing.*.enable_dns_support, [""])[0] : concat(aws_vpc.this.*.enable_dns_support, [""])[0]
}

output "vpc_enable_dns_hostnames" {
  description = "Whether or not the VPC has DNS hostname support"
  value       = var.vpc_id != null ? concat(data.aws_vpc.existing.*.enable_dns_hostnames, [""])[0] : concat(aws_vpc.this.*.enable_dns_hostnames, [""])[0]
}
...
output "vpc_main_route_table_id" {
  ...
  value       = var.vpc_id != null ? concat(data.aws_vpc.existing.*.main_route_table_id, [""])[0] : concat(aws_vpc.this.*.main_route_table_id, [""])[0]
}

output "vpc_ipv6_association_id" {
  description = "The association ID for the IPv6 CIDR block"
  value       = var.vpc_id != null ? concat(data.aws_vpc.existing.*.ipv6_association_id, [""])[0] : concat(aws_vpc.this.*.ipv6_association_id, [""])[0]
}

output "vpc_ipv6_cidr_block" {
  description = "The IPv6 CIDR block"
  value       = var.vpc_id != null ? concat(data.aws_vpc.existing.*.ipv6_cidr_block, [""])[0] : concat(aws_vpc.this.*.ipv6_cidr_block, [""])[0]
}
